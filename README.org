...
This emacs package provides an interface that funcitons as a wrapper around tools (see below) that 1) download a website and all its assets as a single HTML file, and 2) convert that file into an Org mode file.
The main contribution for me is the ability to quickly generate multiple Org files of the same source with different converters and converter parameters.
